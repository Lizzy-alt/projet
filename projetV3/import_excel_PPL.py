...
def import_excel_to_db(excel_file, table_name='PPL', database_name='game.db'):
    try:
        ...
        for index, row in df.iterrows():
            cursor.execute(
                f'INSERT INTO {table_name} (Nom, Prenom, Date_obtention) VALUES (?, ?, ?)',
                (str(row['Nom']), str(row['Prenom ']), str(row['date_obtention']))  # Noms exacts des colonnes
            )
