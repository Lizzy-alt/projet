...
def import_excel_to_db(excel_file, table_name='Boite_mail', database_name='game.db'):
    try:
        ...
        df['Date'] = df['Date'].astype(str)  # Majuscule pour Date
        df['Message'] = df['Message'].astype(str)  # Majuscule pour Message
        ...
        for index, row in df.iterrows():
            cursor.execute(
                f'INSERT INTO {table_name} (date, message) VALUES (?, ?)',
                (str(row['Date']), str(row['Message']))  # Majuscules pour les noms des colonnes
            )
